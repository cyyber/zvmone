...
/// This file contains non-mainstream QRVM unit tests not matching any concrete category:
/// - regression tests,
/// - tests from fuzzers,
/// - qrvmone's internal tests.

#include "qrvm_fixture.hpp"

using qrvmone::test::qrvm;

TEST_P(qrvm, qrvmone_loaded_program_relocation)
{
    // The bytecode of size 2 will create qrvmone's loaded program of size 4 and will cause
...
TEST_P(qrvm, qrvmone_block_stack_req_overflow)
{
...
TEST_P(qrvm, qrvmone_block_max_stack_growth_overflow)
{
...
TEST_P(qrvm, qrvmone_block_gas_cost_overflow_create)
{
...
TEST_P(qrvm, qrvmone_block_gas_cost_overflow_balance)
{
...
TEST_P(qrvm, loop_full_of_jumpdests)
{
    ...
    execute(code);
    EXPECT_GAS_USED(QRVMC_SUCCESS, 7987882);
}

TEST_P(qrvm, jumpdest_with_high_offset)
{
    for (auto offset : {3u, 16383u, 16384u, 32767u, 32768u, 65535u, 65536u})
    {
        ...
        execute(code);
        EXPECT_EQ(result.status_code, QRVMC_SUCCESS) << "JUMPDEST at " << offset;
    }
}
